...
| # | 决策 | 推荐 |
|---|---|---|
| Q1 | 脏树处理 | 先提交为 baseline，再开 worktree |
| Q2 | 操作人模型 | 无登录：本地"当前操作人"+ 自由文本责任人（用户与权限是 P2） |
| Q3 | 复核语义 | 每条发现独立复核；全部链头发现均有 Human Review 后案件才 COMPLETED |
| Q4 | 规则管理深度 | 版本化参数 + 条款立场 + 发布门禁；逻辑保持 TS，运行时读已发布参数 |
| Q5 | 验证案例数据源 | golden-set.ts 保持 canonical（CI 基准不变），seed 物化到 DB 供 UI |
| Q6 | 整改项来源 | 发现被 ACCEPTED 时自动生成整改项 |
| Q7 | 演示数据 | E 阶段扩展 seed：规则 5 条已发布、验证运行、各状态整改项 |
